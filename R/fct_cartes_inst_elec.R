...
#' @param fil un motif de texte a retrouver dans le code typo des installations electrique
...
couche_fil <- function(fil, code_ter, col, leg = FALSE) {
  if(nrow(mes_instal(fil, code_ter)) > 0) {
    ...
    carte_fil <- mapview::mapview(dta, zcol="type", col.regions=col_registre[[col]], map.types = c("CartoDB.Positron"),
